...
def _default_workspace():
    ...
    Honors the active persona: when config has a non-empty `active_persona`,
    the default memory workspace (which the process-wide ConversationStore and
    any config-less MemoryManager fall back to) is rooted at
    `{agent_workspace}/personas/{persona}/`. This isolates short-term
    conversation history per persona without touching session_id plumbing —
    persona switches require a restart, so the lazily-built singleton always
    picks up the persona that is active for this process.
    """
    ...
    if persona:
        return os.path.join(base, "personas", persona)
    return base
