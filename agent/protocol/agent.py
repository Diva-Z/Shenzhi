...
class Agent:
    ...
    def get_full_system_prompt(self, skill_filter=None) -> str:
        ...
        try:
            ...
            # When a persona/channel scope is active, the initializer attaches a
            # provider that loads shared files (RULE.md) from the root workspace
            # and overrides AGENT/USER/MEMORY with the active scope's copies.
            # Falling back to a plain load from workspace_dir would drop RULE.md
            # (which lives at the shared root, not the persona dir).
            provider = getattr(self, "context_files_provider", None)
            if callable(provider):
                context_files = provider()
            else:
