...
def _get_channel(channel_type):
    """Return the running channel instance if available, else create a new one."""
    try:
        from app import get_channel_manager
        mgr = get_channel_manager()
        if mgr:
            ch = mgr.get_channel(channel_type)
            if ch is not None:
                return ch
    except Exception:
        pass
...
def _execute_agent_task(task: dict, agent_bridge) -> bool:
    ...
    try:
        ...
        if channel_type == "web":
            ...
            context["telegram_chat_id"] = receiver
            context["msg"] = None
...
def _execute_send_message(task: dict, agent_bridge) -> bool:
    ...
    try:
        ...
        # Mark as scheduled so channels (e.g. telegram follow-up) can treat
        # proactive pushes differently from real conversation replies.
        context["is_scheduled_task"] = True
