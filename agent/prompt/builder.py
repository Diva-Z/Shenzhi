...
def _build_tooling_section(tools: List[Any], language: str) -> List[str]:
    ...
    if is_en:
        core_summaries = {
            ...
            "conversation_search": "search original chat logs",
            "conversation_get": "read original chat context",
        ...
        core_summaries = {
            ...
            "memory_search": "搜索记忆",
            "memory_get": "读取记忆内容",
            "conversation_search": "搜索原始聊天记录",
            "conversation_get": "读取原始聊天上下文",
...
def _build_memory_section(memory_manager: Any, tools: Optional[List[Any]], language: str) -> List[str]:
    ...
    if language == "en":
        lines = [
            ...
            "- SQLite conversation log: original per-message records; query it with `conversation_search` / `conversation_get` only when exact evidence is needed",
        ...
        lines = [
            ...
            "- SQLite 对话原文库：逐条保存的原始聊天记录；只有需要精确原话/证据时才用 `conversation_search` / `conversation_get` 查询",
...
def _build_runtime_section(runtime_info: Dict[str, Any], language: str) -> List[str]:
    ...
    time_directive = (
        "When asked about the time or date, you MUST use the Current time above — "
        "never guess, and never reuse an old timestamp from earlier in the chat."
        if is_en else
        "⏰ 涉及时间、日期、“几点了”之类的问题，一律以上面这个「当前时间」为准，"
        "不要凭印象或聊天记录里的旧时间编造。"
    )
