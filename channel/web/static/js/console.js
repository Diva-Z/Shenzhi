/* =====================================================================
   沈知 Console - Main Application Script
   ===================================================================== */
...
let appConfig = { use_agent: false, title: '沈知', subtitle: '', providers: {}, api_bases: {} };
...
function startSSE(requestId, loadingEl, timestamp, titleInfo, replayItems) {
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <img src="assets/logo.jpg" alt="沈知" class="w-8 h-8 rounded-lg flex-shrink-0">
    ...
    function processSSEItem(item) {
            if (item.type === 'reasoning') {
                ...
                contentEl.innerHTML = renderMarkdown(accumulatedText.replace(/\[MSG\]/g, '\n\n'));
                scrollChatToBottom();
                ...
                const isSubsequentDone = done; // true if backend sent a split message
                done = true;
                clearOwnerRequest();
                resetSendBtnSendMode();
                ...
                // Split any remaining [MSG] markers (client-side fallback).
                const msgParts = finalText
                    ? finalText.split('[MSG]').map(p => p.trim()).filter(p => p)
                    : [];
                const firstPart = msgParts.length > 0 ? msgParts[0] : finalText;
                const extraParts = msgParts.slice(1);

                if (isSubsequentDone || (!botEl && firstPart)) {
                    // Subsequent split message, or non-streaming first message: new bubble
                    if (loadingEl) { loadingEl.remove(); loadingEl = null; }
                    addBotMessage(firstPart, new Date((item.timestamp || Date.now() / 1000) * 1000), isSubsequentDone ? undefined : requestId);
                } else if (botEl) {
                    contentEl.classList.remove('sse-streaming');
                    if (firstPart) contentEl.innerHTML = renderMarkdown(firstPart);
                    contentEl.dataset.rawMd = firstPart || '';
                    const copyBtn = botEl.querySelector('.copy-msg-btn');
                    if (copyBtn && firstPart) copyBtn.style.display = '';
                    applyHighlighting(botEl);
                }

                extraParts.forEach((part, idx) => {
                    setTimeout(() => { addBotMessage(part, new Date()); scrollChatToBottom(); }, 800 * (idx + 1));
                });
                ...
                renderBotSpeakerButton(botEl, firstPart);
                scrollChatToBottom();
...
function newChat(optimistic = true) {
    ...
    ws.innerHTML = `
        <img src="assets/logo.jpg" alt="沈知" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || '沈知'}</h1>
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `沈知 ${APP_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = '沈知';
    });
    chatInput.focus();
}
