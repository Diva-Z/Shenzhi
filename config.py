...
def load_config():
    ...
    # print ASCII logo
    logger.info("  ____  _                 _____ _     _ ")
    logger.info(" / ___|| |__   ___ _ __  |__  /| |__ (_)")
    logger.info(" \\___ \\| '_ \\ / _ \\ '_ \\   / / | '_ \\| |")
    logger.info("  ___) | | | |  __/ | | | / /_ | | | | |")
    logger.info(" |____/|_| |_|\\___|_| |_|/____||_| |_|_|")
    logger.info("")
    # SHENZHI_CONFIG lets a second instance load an alternate config file
    # (e.g. config-haiyang.json) so multiple personas can each run as their own
    # bot in their own process. Defaults to ./config.json (unchanged behaviour).
    # COW_CONFIG is still honoured for backward compatibility.
    config_path = (os.environ.get("SHENZHI_CONFIG", "").strip()
                   or os.environ.get("COW_CONFIG", "").strip()
                   or "./config.json")
