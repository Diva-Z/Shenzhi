...
Session persistence and model context are different layers: the database keeps original per-message records, but the model does not automatically load all history every turn. When exact wording needs to be verified, the Agent uses:

- `conversation_search`: search keywords or phrases in the SQLite raw chat log
- `conversation_get`: read neighbouring messages by the matched `session_id + seq`

This layer is for questions like "what exactly did I say before?", "how did you reply then?", and "did we ever talk about X?". General preferences, long-term facts, and relationship changes still belong in `MEMORY.md`, daily memory, and `memory_search`.
