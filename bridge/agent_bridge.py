...
class AgentBridge:
    ...
    def get_agent(self, session_id: str = None, channel_type: str = None) -> Optional[Agent]:
        """
        Get agent instance for the given session

        Args:
            session_id: Session identifier (e.g., user_id). If None, returns default agent.
            channel_type: Channel name (e.g. "telegram", "weixin"). Used to isolate
                          per-channel memory on first initialisation of a session.
        ...
        if session_id not in self.agents:
            self._init_agent_for_session(session_id, channel_type=channel_type)
    ...
    def _init_agent_for_session(self, session_id: str, channel_type: str = None):
        """Initialize agent for a specific session"""
        agent = self.initializer.initialize_agent(session_id=session_id, channel_id=channel_type)
    ...
    def agent_reply(self, query: str, context: Context = None, 
        ...
        try:
            ...
            agent = self.get_agent(session_id=session_id, channel_type=channel_type or None)
            if not agent:
